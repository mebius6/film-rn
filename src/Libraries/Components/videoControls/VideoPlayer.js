...
import AudioVideoSwither from './AudioVideoSwitcher';
...
export default class VideoPlayer extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      videoLayoutHeight: 0,
      maskTop: 0,
      is_full_screen: this.props.is_full_screen || false,
    };
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    // if (
    //   prevState.is_full_screen != nextProps.is_full_screen &&
    //   nextProps.paused === true
    // ) {
    //   this.needResetPosition = true;
    // }
    if (nextProps.is_full_screen !== prevState.is_full_screen) {
      return {
        is_full_screen: nextProps.is_full_screen,
      };
    }
    // if (!_.isEqual(nextProps.videoUrl, prevState.videoUrl)) {
    //   console.log('nextProps.videoSource', nextProps.videoSource);
    //   return {
    //     mediaType: 'video',
    //   };
    // }
  ...
  _onLoad(data = {}) {
    ...
    const {lastPlayedTime} = this.state;
    if (lastPlayedTime > 0) {
      this.seekTo(lastPlayedTime);
    }
  }
  ...
  onAudioVideoSwitch() {
    const {videoSource, audioSource} = this.props;
    const {currentTime, mediaType} = this.state;
    if (mediaType === 'video') {
      this.setState({
        mediaType: 'audio',
        source: audioSource,
    ...
    return (
      <Animated.View
        ...
        <ImageBackground
          ...
          <View style={styles.controls.topControlGroup}>{backControl}</View>
        </ImageBackground>
      </Animated.View>
    );
  }
  ...
  renderBack() {
    ...
    return this.renderControl(backButton, this.events.onBack, {
      width: 80,
      height: 80,
  ...
  renderFullScreenButton = () => {
    let label = this.props.is_full_screen
      ? require('./assets/quit_full.png')
      : require('./assets/go_full.png');
    return (
      <TouchableHighlight
        underlayColor="transparent"
        activeOpacity={0.3}
   ...
   */
  render() {
    const {is_full_screen, mediaType} = this.state;
    const {videoUrl, audioUrl} = this.props;
    return (
      <TouchableWithoutFeedback
        ...
        style={[
          ...
          is_full_screen ? styles.player.androidVideoCls : null,
        ]}>
        <View
          style={[
            styles.player.container,
          ...
          {aspect_ration < 1.5 && this.props.videoPattern === 2 && (
            <View
              style={[
          ...
          <Video
            ...
            style={[
              this.props.videoPattern === 1 && styles.player.video,
              this.styles.videoStyle,
              is_full_screen ? styles.player.androidVideoPlayerCls : null,
            ]}
            source={{
              uri: mediaType === 'video' ? videoUrl : audioUrl,
            }}
            onLayout={ev => {
              ...
              if (
                this.props.videoPattern === 2 &&
                aspect_ration < 1.5 &&
                ev.nativeEvent.layout.y > 0
              ) {
                this.setState({
          ...
          {/* {this.props.videoPattern === 1 ? (
            <AudioVideoSwither
              onSwitch={() => this.onAudioVideoSwitch()}
              title={this.state.mediaType === 'video' ? '音频' : '视频'}
            />
          ) : null} */}
        </View>
      </TouchableWithoutFeedback>
      ...
      <TouchableWithoutFeedback onPress={this.methods.togglePlayPause}>
        <View
          ...
          <Image
            ...
            source={require('./assets/icn_play.png')}
          />
        </View>
      </TouchableWithoutFeedback>
    );
